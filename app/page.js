  ...
  return (
    <main style={{background:"#020617",color:"white",minHeight:"100vh",fontFamily:"Arial",padding:"40px"}}>
      <section style={{textAlign:"center",padding:"60px 20px"}}>
        <h1 style={{fontSize:"60px",marginBottom:"20px"}}>Shriram Contact Center</h1>
      ...
      <section style={{marginTop:"60px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns":"repeat(auto-fit,minmax(250px,1fr))",gap:"20px"}}>
          {services.map((service,index)=>(
            <div key={index} style={{background:"#0f172a",padding:"25px",borderRadius:"20px"}}>
              <h3>{service}</h3>
            </div>
          ))}
        </div>
      ...
      <section style={{marginTop:"80px"}}>
        <h2 style={{fontSize:"40px",marginBottom:"30px"}}>Packages</h2>

        <div style={{display:"grid",gridTemplateColumns":"repeat(auto-fit,minmax(250px,1fr))",gap:"20px"}}>
          {packages.map((pkg,index)=>(
            <div key={index} style={{background:"#0f172a",padding:"30px",borderRadius:"20px"}}>
              <h3 style={{fontSize:"28px"}}>{pkg.name}</h3>
              <p style={{fontSize:"24px",color:"#60a5fa"}}>{pkg.price}</p>
            </div>
          ))}
        </div>
      </section>
